...
class Tokenizer:
    ...
    def encode(self, text, bos=False, eos=False, allowed_special=set(), disallowed_special=()):
        if bos:
            ...
            tokens = []
        tokens += self.model.encode(text=text,allowed_special=allowed_special, disallowed_special=disallowed_special)
        if eos:
...
def clean_text(text, header_end="assistant<|end_header_id|>\n\n"):
    ...
    if index != -1:
        return text[index + len(header_end):].strip() # remove whitespaces
    else:
        return text
