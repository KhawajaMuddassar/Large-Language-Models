...
def load_dataframes():
    df_train = pd.read_csv("SentimentClassifier/data/processed/train.csv")
    df_val = pd.read_csv("SentimentClassifier/data/processed/val.csv")
    df_test = pd.read_csv("SentimentClassifier/data/processed/test.csv")
...
def eval(model, X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    print(f"Training Accuracy: {accuracy_train*100:.2f}%, Validation Accuracy: {accuracy_val*100:.2f}%, Test Accuracy: {accuracy_test*100:.2f}%")    
