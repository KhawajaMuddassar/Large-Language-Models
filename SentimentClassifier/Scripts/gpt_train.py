...
from utils import calc_loss_batch, evaluate_model, calc_accuracy_loader, instantiate_model
import argparse
...
def train_classifier(model, train_loader, val_loader, optimizer, device, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, target_batch in train_loader:
            optimizer.zero_grad() 
            loss = calc_loss_batch(input_batch, target_batch, model, device,model_type=model_type,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Which GPT model to use. Options: 'gpt2-small', 'gpt2-medium',"
            " 'gpt2-large', 'gpt2-xl'."
        )
    )
    parser.add_argument(
    ...
    if args.model_size is None:
        ...
        user_input = input("Choose Model size (gpt2-small, gpt2-medium, gpt2-large, gpt2-xl): ").strip()
        if user_input in available_models:
            args.model_size = user_input
    ...
    train_losses, val_losses, train_accs, val_accs, examples_seen = train_classifier(
        ...
        average_embeddings=args.average_embeddings,model_type=args.model_type
    )
    ...
    os.makedirs('SentimentClassifier/model/gpt_model', exist_ok=True)
    torch.save(model.state_dict(), "SentimentClassifier/model/gpt_model/{args.model_type}classifier.pth")
    ...
    train_accuracy = calc_accuracy_loader(
        ...
        trainable_token_pos=args.trainable_token_pos, average_embeddings=args.average_embeddings,model_type=args.model_type
    )
    val_accuracy = calc_accuracy_loader(
