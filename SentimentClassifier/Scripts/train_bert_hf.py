...
from utils import calc_loss_batch, calc_accuracy_loader, evaluate_model
...
def train_classifier(model, train_loader, val_loader, optimizer, device, num_epochs,
                            eval_freq, eval_iter, max_steps=None, model_type='roberta'):
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, attention_mask_batch, target_batch in train_loader:
            ...
            if global_step % eval_freq == 0:
                train_loss, val_loss = evaluate_model(
                    model, train_loader, val_loader, device, eval_iter,model_type=model_type)
        ...
        train_accuracy = calc_accuracy_loader(train_loader, model, device, num_batches=eval_iter,model_type=model_type)
        val_accuracy = calc_accuracy_loader(val_loader, model, device, num_batches=eval_iter,model_type=model_type)
...
if __name__ == "__main__":
    ...
    if args.model is None:
        ...
        user_input = input("Choose Model (bert, distilbert, roberta): ").strip()        
        if user_input in available_models:
            args.model = user_input
            
    if args.model == "distilbert":
        print(f"\nDownloading {args.model} weights...\n")
    ...
    base_path = Path("SentimentClassifier/data/processed")
    ...
    train_losses, val_losses, train_accs, val_accs, examples_seen = train_classifier(
        ...
        max_steps=None,model_type=args.model
    )

    end_time = time.time()
    execution_time_minutes = (end_time - start_time) / 60
    print(f"Training completed in {execution_time_minutes:.2f} minutes.")
    os.makedirs('SentimentClassifier/model/bert_model', exist_ok=True)
    torch.save(model.state_dict(), "SentimentClassifier/model/bert_model/{args.model}classifier.pth")

    print("\nEvaluating on the full datasets using {args.model} ...\n")

    train_accuracy = calc_accuracy_loader(train_loader, model, device,model_type=args.model)
    val_accuracy = calc_accuracy_loader(val_loader, model, device,model_type=args.model)
    test_accuracy = calc_accuracy_loader(test_loader, model, device,model_type=args.model)
