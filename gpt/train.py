...
from gpt.utils import evaluate_model, generate_print, device
from gpt.utils import batch_loss, tokenizer, plot_losses, lr_behavier
...
def main(gpt_config, settings,device, tokenizer):    
    ...
    # Download Data    
    #*********************************
    file_path = 'gpt/rawdata/alice_in_wonderland.txt'
...
if __name__ == "__main__":
    ...
    plt.savefig("gpt/selftrainedmodel/loss.pdf")

    # Save and load model
    torch.save(model.state_dict(), "gpt/selftrainedmodel/GPT2_small.pth")
    model = GPTModel(gpt_config)
    model.load_state_dict(torch.load("gpt/selftrainedmodel/GPT2_small.pth", weights_only=True))